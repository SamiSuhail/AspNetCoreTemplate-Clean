...
{
    private const string MessageSubject = "Go2Gether Email Change Confirmation";
    ...
    public async Task Consume(ConsumeContext<ChangeEmailMessage> context)
    {
        ...
        await emailSender.Send(username, oldEmail, MessageSubject, messageTextOldEmail, context.CancellationToken);

        var messageTextNewEmail = string.Format(MessageTemplate, BaseConfirmationConstants.ExpirationTimeMinutes, newEmailCode);
        await emailSender.Send(username, newEmail, MessageSubject, messageTextNewEmail, context.CancellationToken);
    }
}
